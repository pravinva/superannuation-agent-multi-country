...
🏛️ Age Pension Impact:
   ...
   • Pension change: ${pension_impact.get('pension_increase', 0):,.2f}
...
"""
    ...
    return {
        ...
        "citations": [
            ...
            "Unity Catalog: calculate_tax_implications, check_age_pension_impact, project_retirement_income"
        ]
    }

# ============================================================================
# USA - Unity Catalog Functions
# ============================================================================

def calculate_401k_usa(user_data):
    """USA 401(k) calculator using UC functions"""
    member_id = user_data.get('member_id')
...
🇺🇸 Social Security Impact:
   • Estimated annual SS benefit: ${ss_impact.get('estimated_annual_ss', 0):,.2f}
   • Combined retirement income: ${ss_impact.get('combined_retirement_income', 0):,.2f}
   • Income assessment: {ss_impact.get('income_adequacy', 'N/A')}
...
"""
    ...
    return {
        ...
        "citations": [
            COUNTRY_REGULATIONS["USA"][0],
            "Unity Catalog: calculate_401k_withdrawal, check_social_security_impact, project_401k_balance"
        ]
    }

# ============================================================================
# UK - Unity Catalog Functions
# ============================================================================

def calculate_uk_pension(user_data):
    """UK pension calculator using UC functions"""
    member_id = user_data.get('member_id')
    balance = user_data.get('super_balance', 0)
    age = user_data.get('age', 65)
...
🇬🇧 State Pension Impact:
   • State pension age: {state_pension.get('state_pension_age', 67)}
   • Eligible: {'Yes' if state_pension.get('eligible_for_state_pension') else 'No'}
   • Annual state pension: £{state_pension.get('annual_state_pension', 0):,.2f}
   • Combined income: £{state_pension.get('combined_annual_income', 0):,.2f}
   • Assessment: {state_pension.get('income_assessment', 'N/A')}
...
"""
    ...
    return {
        ...
        "citations": [
            COUNTRY_REGULATIONS["UK"][0],
            "Unity Catalog: calculate_uk_pension_withdrawal, check_uk_state_pension, project_uk_pension_balance"
        ]
    }

# ============================================================================
# INDIA - Unity Catalog Functions
# ============================================================================

def calculate_india_pf(user_data):
    """India Provident Fund calculator using UC functions"""
    member_id = user_data.get('member_id')
    balance = user_data.get('super_balance', 0)
    age = user_data.get('age', 65)
...
💰 Recommended Annual Withdrawal: ₹{withdrawal_amount:,.2f}
   ...
   • TDS applicable: {'Yes' if tax_info.get('tds_applicable') else 'No'}
   • Tax amount: ₹{tax_info.get('tax_amount', 0):,.2f}
   • Net withdrawal: ₹{tax_info.get('net_withdrawal', withdrawal_amount):,.2f}

🇮🇳 Pension Impact:
   • Estimated monthly pension: ₹{pension_impact.get('estimated_monthly_pension', 0):,.2f}
   • Annual pension income: ₹{pension_impact.get('annual_pension_income', 0):,.2f}
   • Income adequacy: {pension_impact.get('income_adequacy', 'N/A')}
...
"""
    ...
    return {
        ...
        "citations": [
            COUNTRY_REGULATIONS["India"][0],
            "Unity Catalog: calculate_epf_withdrawal, check_india_pension_impact, project_india_pf_balance"
        ]
    }
