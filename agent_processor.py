# agent_processor.py
"""
Agent query processing with full logging and validation
Integrates Unity Catalog tools and MLflow tracking
"""
...
def agent_query(user_id, country, query_str, extra_context, session_id, 
    ...
    """
    Process agent query with full logging and validation

    Args:
        user_id: User identifier
        country: Country code (Australia, USA, UK, India)
        query_str: User's query string
        extra_context: Member profile data (dict with member_id, age, balance, etc.)
        session_id: Session identifier
        judge_llm_fn: Optional judge LLM function for validation
        mlflow_experiment_path: MLflow experiment path (defaults to prod)

    Returns:
        Tuple of (answer, citations, result_dict, judge_resp, judge_verdict, error_info)
    """
    ...
    try:
        ...
        # Extract results - updated for new structure
        amount = calc_result.get('amount', 0)
        answer = calc_result.get('summary', f"Estimated annual withdrawal: ${amount:,.2f}")
    ...
    try:
        log_query_event(
            ...
            cost=0.05,  # Replace with actual cost calculation if available
